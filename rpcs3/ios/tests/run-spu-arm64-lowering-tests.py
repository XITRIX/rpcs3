...
def main():
    ...
    out = Path(os.environ.get('SPU_LOWERING_TEST_OUTPUT', os.environ.get('TMPDIR', '/tmp') + '/rpcs3-spu-lowering-tests'))
    ...
    run([*compiler, *shlex.split(llvm('--cxxflags')), '-std=c++20', '-O2', '-I', CORE,
         HERE / 'SPUARM64LoweringFixture.cpp', *shlex.split(llvm('--ldflags', '--libs', 'all', '--system-libs')),
         '-Wl,-rpath,'+llvm('--libdir'), '-o', fixture])
    run([sys.executable, HERE / 'emit-spu-reversal-fixture.py', out / 'reversal.ll'])
    run([fixture, out / 'reversal.ll', out / 'reversal-pipeline.ll', 'pipeline'])
    target = 'arm64-apple-macos15' if platform.system() == 'Darwin' else 'aarch64-unknown-linux-gnu'
    cpu = os.environ.get('SPU_LOWERING_TEST_CPU', 'generic')
    run([Path(llvm('--bindir')) / 'llc', '-O2', '-mtriple='+target, '-mcpu='+cpu, '-filetype=obj',
         out / 'reversal-pipeline.ll', '-o', out / 'reversal.o'])
    run([*compiler, '-std=c++20', '-O2', '-Wall', '-Wextra', '-Werror', '-I', out,
         HERE / 'SPUARM64LoweringTests.cpp', out / 'reversal.o', '-o', out / 'reversal'])
    run([out / 'reversal', *sys.argv[1:]])
