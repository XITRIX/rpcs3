...
const std::array catalog{
	...
	SETTING("advanced.silence_logs", "Advanced", "Diagnostics", "Silence all logs", "Suppresses routine RPCS3 log messages. Fatal errors remain visible so a failed game can be diagnosed.", kind::RPCS3_IOS_SETTING_BOOLEAN, g_cfg.misc.silence_all_logs, 0, 1, 1),
	...
	SETTING("experimental.dma_copy_specialization", "Experimental", "SPU DMA", "Specialize small DMA copies", "Uses fixed-size ARM64 copies for 64 to 128 byte SPU DMA transfers while retaining synchronization. Default (Automatic): enabled. Disabled restores the original copy loop. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.dma_copy_specialization, 0, 0, 0),
	SETTING("experimental.texture_hash_hybrid", "Experimental", "RSX Textures", "Hybrid large-texture hash", "Uses an alternate ARM64 hash implementation for texture ranges of at least 4 KiB, checking the same bytes. Default (Automatic): enabled. Disabled restores the original hash. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.texture_hash_hybrid, 0, 0, 0),
	SETTING("experimental.neon_byte_swap", "Experimental", "ARM64 Uploads", "ARM64 byte-swap uploads", "Uses a four-lane NEON path for swapped RSX uploads. Default (Automatic): enabled. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.neon_byte_swap, 0, 0, 0),
	SETTING("experimental.neon_primitive_restart", "Experimental", "ARM64 Uploads", "ARM64 primitive-restart uploads", "Uses NEON min/max and restart filtering for index uploads. Default (Automatic): enabled. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.neon_primitive_restart, 0, 0, 0),
	SETTING("experimental.precomputed_indices", "Experimental", "ARM64 Uploads", "Precomputed non-native indices", "Copies cached quad and triangle-fan index patterns instead of regenerating them for each draw. Default (Automatic): enabled. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.precomputed_indices, 0, 0, 0),
	SETTING("experimental.mobile_spu_scheduling", "Experimental", "SPU", "Mobile SPU compile scheduling", "Uses a nonzero compile-throttle floor on low-core mobile CPUs. Default (Automatic): enabled. Disabled restores upstream scheduling. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.mobile_spu_scheduling, 0, 0, 0),
	SETTING("experimental.fifo_cache_size", "Experimental", "RSX FIFO", "RSX FIFO read cache", "Selects original RPCS3's 1 KiB cache behavior or ARMSX3's 4 KiB cache behavior with fewer refills. Default: 4 KiB. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.fifo_cache_size, 0, 0, 0),
	SETTING("experimental.fifo_idle_mode", "Experimental", "RSX FIFO", "RSX FIFO idle wait", "Default: Wait for event. Follows ARMSX3's eight short CPU pauses followed by ARM event waits, with scheduler yielding as fallback. Yield uses original RPCS3's idle behavior. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.fifo_idle_mode, 0, 0, 0),
	SETTING("experimental.deferred_get_publishing", "Experimental", "RSX FIFO", "Deferred FIFO GET publishing", "Follows ARMSX3 by publishing guest FIFO progress every eighth packet and flushing changed progress before idle or blocking paths. Default (Automatic): enabled. Disabled restores original RPCS3's publication behavior. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.deferred_get_publishing, 0, 0, 0),
	SETTING("experimental.getllar_backoff", "Experimental", "SPU", "GETLLAR mobile backoff", "Memoizes stack classification and stops repeated out-buffer verdicts from suppressing SPU backoff forever. Default (Automatic): enabled. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.getllar_backoff, 0, 0, 0),
	SETTING("experimental.rsx_dma_wait_parking", "Experimental", "RSX", "Park RSX DMA waits", "After a short spin, parks the RSX thread while its offloader drains work. Default (Automatic): enabled. Disabled restores RPCS3's continuous spin. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.rsx_dma_wait_parking, 0, 0, 0),
	SETTING("experimental.vulkan_command_buffer_reclamation", "Experimental", "Vulkan Memory", "Per-command-buffer ring reclamation", "Returns completed Vulkan ring allocations after each command buffer, including loads with no presentation. Default (Automatic): enabled. Disabled restores frame-only reclamation. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.vulkan_command_buffer_reclamation, 0, 0, 0),
	SETTING("experimental.expanded_spu_scratch", "Experimental", "SPU", "Expanded ARM64 SPU scratch", "Reserves 256 KiB instead of RPCS3's 8 KiB for generated ARM64 SPU spills. Default (Automatic): enabled, reserving 256 KiB. Disabled restores 8 KiB. Takes effect on the next boot.", kind::RPCS3_IOS_SETTING_CHOICE, g_cfg.ios_experimental.expanded_spu_scratch, 0, 0, 0),
