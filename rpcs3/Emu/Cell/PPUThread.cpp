...
bool ppu_initialize(const ppu_module<lv2_obj>& info, bool check_only, u64 file_size)
{
	...
	// Subject to change
#ifdef RPCS3_IOS
	constexpr u32 c_modules_per_jit = rpcs3::ios::jit::ppu_modules_per_jit;
#else
...
#ifdef __APPLE__
			// Restore the executable state on macOS. This is a no-op for the
			// iOS mirror-mapped backend because RX addresses are always stable.
			jit_write_protect(true);
#endif
